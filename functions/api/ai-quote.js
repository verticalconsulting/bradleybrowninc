...
async function callCloudflareAI(token, prompt) {
  ...
  const ACCOUNT_ID = "af822065bd416045e3a0e3d07e0648fe" // Bradley Brown Inc Account
  const GATEWAY_NAME = "bradleybrown-ai" // AI Gateway name
...
function processAIResponse(aiText, formData) {
  ...
  const totalCost =
    materialsCost + laborCost + permitsCost + featuresCost + contingency
  ...
  return {
    ...
    aiInsights:
      aiText ||
      "AI analysis provided detailed recommendations based on your specific project requirements.",
  }
}
